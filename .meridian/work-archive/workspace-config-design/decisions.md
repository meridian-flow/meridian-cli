...
## D26 — Stronger Boundary: Repo `.meridian/` for Committed Artifacts Only

**Directive (2026-04-17):** Repo-level `.meridian/` is exclusively for committed/shared project artifacts. All ephemeral/runtime Meridian state moves to user-level `~/.meridian/projects/<project-key>/...`. This is a stronger rule than the prior design's "trending toward" language — there is no split between repo-local and user-level runtime state.

### What Moves to User-Level State

All runtime/ephemeral state moves under `~/.meridian/projects/<project-key>/`:

| Artifact | Old Location | New Location |
|---|---|---|
| Spawn metadata index | `.meridian/spawns.jsonl` | `~/.meridian/projects/<pk>/spawns.jsonl` |
| Session metadata index | `.meridian/sessions.jsonl` | `~/.meridian/projects/<pk>/sessions.jsonl` |
| Per-spawn artifacts | `.meridian/spawns/<spawn-id>/` | `~/.meridian/projects/<pk>/spawns/<spawn-id>/` |
| Cache | `.meridian/cache/` | `~/.meridian/projects/<pk>/cache/` |
| Spawn flocks | `.meridian/spawns.jsonl.flock` | `~/.meridian/projects/<pk>/spawns.jsonl.flock` |

### What Stays in Repo `.meridian/`

Only committed/shared artifacts remain under repo `.meridian/`:

| Artifact | Status |
|---|---|
| `work/`, `fs/`, `work-archive/` | Transitional — awaiting cloud/shared alternative per G8 |
| `.gitignore` | Committed — manages ignore policy for the directory |

**No runtime state remains under repo `.meridian/`.** There is no compatibility carveout or transitional split.

### Runtime-Home Keying

Runtime filesystem state is keyed by `spawn_id` exclusively:

- `~/.meridian/projects/<project-key>/spawns/<spawn-id>/` is the per-run runtime home.
- Primary sessions also receive Meridian `spawn_id`s — this makes the layout uniform regardless of launch origin (CLI spawn vs app primary session).
- `chat_id` is harness-level resumable-session metadata for continuations, NOT a storage key.
- App `session_id` is a URL-addressable alias for `(project_key, spawn_id, repo_root)`, NOT a storage key.

### Harness-Native Profile/Config Projection Support

This stronger boundary directly enables harness-native profile/config projection:

- Isolated `CODEX_HOME` per project: `~/.meridian/projects/<pk>/spawns/<spawn-id>/codex-home/`
- Harness state that needs to be project-scoped but not repo-committed.
- Clear separation between workspace shaping (repo paths) and native home/state isolation (user-level paths).

### Harness Workspace Shaping Conclusions (Preserved)

Per D25, harness workspace shaping mechanisms remain as documented:

- **Codex:** `--cd` (primary root) + `--add-dir` (extra writable roots) for workspace shaping; `CODEX_HOME` for native home/state isolation only, not extra workspace roots. Fresh isolated `CODEX_HOME` environments need auth bootstrap.
- **OpenCode:** Launch root plus config/permissions like `permission.external_directory` for extra-path access. No dedicated `--add-dir` flag.
- **Claude:** `--add-dir` flags for workspace roots.

### Compatibility/Migration

This is a breaking change to state layout. The `app-session-architecture` design already assumes this layout and can proceed without modification. Existing `.meridian/spawns.jsonl` and `.meridian/spawns/` state from prior runs will not be visible under the new layout.

**Migration strategy:** None. Per CLAUDE.md "No backwards compatibility needed." Fresh runs use the new layout.

### Spec/Architecture Touchpoints

- `requirements.md` G7 and G9 — updated to reflect stronger rule
- `design/architecture/overview.md` — cross-cutting decisions updated
- `design/architecture/paths-layer.md` — user-level state section updated
- `design/feasibility.md` FV-13 — updated for implementation-ready status
- `app-session-architecture` — already aligned; this design is authoritative source

### Follow-On Alignment: `app-session-architecture`

The `app-session-architecture` design (`../.meridian/work/app-session-architecture/`) already assumes the stronger boundary:

- User-level runtime state at `~/.meridian/projects/<project_key>/spawns/<spawn_id>/`
- Spawn ID as universal runtime-home key
- `chat_id` and `session_id` as metadata only

No changes needed to `app-session-architecture`. The relationship:

- **`workspace-config-design`** is the authoritative source for the boundary rule (D26), paths-layer design, and harness workspace shaping conclusions (D25).
- **`app-session-architecture`** depends on and implements the user-level state layout from this design.

Implementation should treat `workspace-config-design/decisions.md` D26 as the governing decision for both designs.

## D27 — MERIDIAN_HOME as Product-Level Concept (2026-04-17)

**Directive (2026-04-17):** Meridian SHALL have an explicit `MERIDIAN_HOME` environment variable and product-level concept that names the user-level runtime-state root. This consolidates D24 and D26 into a single named concept with explicit platform semantics.

### Why This Decision

Prior designs buried the runtime-home concept under "workspace topology" framing. Windows compatibility work tried to centralize storage/path policy in a broad `platform/*` layer, duplicating what should be Meridian-owned state topology. This decision makes the concept first-class so downstream work can import it directly.

### Platform Defaults

| Platform | Default Value | Rationale |
|----------|---------------|-----------|
| Unix (macOS, Linux) | `~/.meridian/` | XDG-adjacent dotfile convention |
| Windows | `%LOCALAPPDATA%\meridian\` | Windows user-local app data convention |

### Resolution Order

1. **`MERIDIAN_HOME` environment variable** — if set, use it directly
2. **Platform default** — per table above

### Relationship to D24/D26

- **D24** introduced project-key for scoped state — now keyed under `$MERIDIAN_HOME/projects/<project-key>/`
- **D26** established the stronger boundary — repo `.meridian/` is committed artifacts only, runtime state lives under `MERIDIAN_HOME`
- **D27** names the product-level concept and adds explicit platform defaults

### Ownership Model

The ownership model is explicit and reusable by Windows compatibility:

| Owner | What it owns | Platform portability |
|-------|--------------|---------------------|
| Repo `.meridian/` | Committed artifacts (`work/`, `fs/`) | Path under repo root |
| `MERIDIAN_HOME` | Runtime state (spawns, sessions, cache) | Platform-defaulted, overridable |
| `ProjectPaths` | Repo-root policy files | Path under repo root |
| Harness adapters | Per-spawn home/config projection, harness-native storage | Adapter decides |

### Windows Compatibility Implications

This decision is the foundation for Windows compatibility:

1. **Windows uses `%LOCALAPPDATA%\meridian\`** — a native Windows convention
2. **No broad platform layer** — platform code owns primitives (locking, IPC), not Meridian state topology
3. **Harness storage stays harness-owned** — Windows harness support is per-adapter

The Windows design should import this boundary model, then focus on platform primitives.

### Spec/Architecture Touchpoints

- `requirements.md` — reframed with Tier 1 (Windows dependency) and Tier 2 (workspace topology)
- `design/spec/runtime-boundaries.md` — new spec leaf (HOME-1)
- `design/architecture/runtime-home.md` — new architecture leaf (A00)
- `design/architecture/overview.md` — updated to reference A00 first
- `design/architecture/paths-layer.md` — updated to reference `MERIDIAN_HOME`
...
- **Planner**: use D1–D28 as constraints that survive into the plan. Key decisions:
  - **D28** is the foundation — UUID-based project identity, simplified model
  - **D28 supersedes D24, D26, D27** — those described the complex derivation model
  - **D2, D3** govern refactor sequencing
  - **D17–D21** govern R06 typed-pipeline invariants
- **Windows compatibility work**: import D28 directly as the runtime-home boundary model. UUID + platform defaults. Focus on platform primitives, not derivation algorithms.
- **Future rounds**: when a new prior-round feedback file gets produced, append a new section here rather than rewriting these rows.
...
## D24 — Project-Key for User-Level Runtime State

**Directive (2026-04-17):** Introduce a stable `project-key` identifier for project-scoped runtime state stored under the user-level Meridian home (`~/.meridian/`).

### Core Design

1. **Project-key concept:** A stable identifier derived from the active `.meridian/` directory. Used to key user-level runtime state so artifacts survive repository moves and maintain identity across sessions.

2. **User-level runtime path layout:**
   ```
   ~/.meridian/
     projects/
       <project-key>/
...
3. **Spawn-ID as the universal runtime-home key:** Runtime homes, configs, and per-spawn state are keyed by `spawn_id` only. Primary sessions also receive Meridian `spawn_id`s — this makes the storage layout uniform regardless of whether a session originated from `meridian spawn` or direct primary launch.

4. **Sessions remain metadata/index only:** `chat_id` is a resumable-session identifier used for session linkage, fork lineage, and conversation continuity. It is NOT a runtime-home storage key. Session rows in `sessions.jsonl` are an index concept pointing to spawn-owned artifacts.

5. **Purpose note:** `project-key` exists specifically to support project-scoped runtime-state designs, particularly harness-native profile/config projection where Meridian manages isolated harness state per project (e.g., `CODEX_HOME` per project).

### What This Does NOT Change

- Repo-level state under `.meridian/` (spawns.jsonl, sessions.jsonl, work artifacts) remains unchanged.
- The `workspace.local.toml` workspace topology design is unaffected.
- Launch composition (A04/A06) is unaffected.

### Future Integration Point

When harness-native config projection lands (e.g., isolated `CODEX_HOME` per project), the runtime-state directory at `~/.meridian/projects/<project-key>/` becomes the natural home for projected harness state, ensuring project-scoped isolation without polluting the shared harness home.

## D28 — UUID Simplification (2026-04-17)

**Directive (2026-04-17):** Replace the complex `project-key` derivation model with a simple UUID stored in `.meridian/id`.

### Why This Change

The prior design proposed deriving a `project-key` from:
- Git remote URL hashing (for git repos)
- Canonicalized path hashing (fallback)
- Platform-specific normalization rules

This was fragile:
- Git remote detection fails for non-git projects
- Path canonicalization has edge cases across platforms (symlinks, case sensitivity, network mounts)
- Neither approach reliably survives project moves
- The derivation algorithm added complexity without proportional value

### The Simpler Model

1. **Project UUID** — stored in `.meridian/id`, generated once, moves with project folder
2. **Ephemeral user state** — `~/.meridian/projects/<UUID>/` holds spawns, sessions, cache
3. **No derivation** — no git remote hashing, no path normalization, no algorithm

### What This Changes

**Removed:**
- `project-key` derivation from git remote or path hash
- Complex normalization rules for path canonicalization
- Platform-specific path handling beyond basic defaults
- The "stable across moves via derivation algorithm" claim

**Kept:**
- Windows platform defaults (`%LOCALAPPDATA%\meridian\`)
- The ownership model (project vs user state)
- Spawn-ID as universal runtime-home key
- The basic directory structure concept

### Tradeoffs

**Downside:** Cloning a repo creates a new UUID (the `id` file isn't typically committed). Ephemeral state doesn't transfer across clones.

**Why acceptable:** Ephemeral state (spawns, sessions) is per-machine anyway. Sharing spawn history across clones was never a real use case. Work artifacts (`work/`, `fs/`) that should be shared are committed, not keyed by project identity.

### Target Audience

Biomedical researchers on Windows who just want it to work. They don't care about git, path normalization, or project identity algorithms. They move project folders around and expect things to keep working.

### Spec/Architecture Updates

- `design/spec/runtime-boundaries.md` — rewritten for UUID model
- `design/architecture/runtime-home.md` — rewritten for UUID model
- `design/architecture/paths-layer.md` — simplified, removed derivation logic
- `requirements.md` — updated to reflect simpler scope
- Prior decisions D24, D26, D27 are superseded by this simpler model

## D29 — Planning Reset for D28 (2026-04-17)

**Directive (2026-04-17):** Replace the old remaining-phase execution plan with
a smaller two-phase runtime-home plan while preserving the completed
config-surface convergence phase.

### Why This Change

The previous remaining-phase plan still targeted workspace topology,
`workspace.local.toml`, and launch projection. D28 removed that scope. Keeping
those blueprints would force coders to implement work the design no longer
calls for.

### Chosen Plan Shape

1. Preserve `phase-1-config-surface-convergence` as complete and authoritative.
2. Add one foundation phase for UUID/user-root helper ownership and repo/user
   path separation.
3. Add one integration phase for high-level caller migration and smoke proof.

### Alternatives Rejected

- **Keep the old phase 2 / phase 3 files and reinterpret them.**
  Rejected because their scope language is dominated by workspace topology and
  harness projection behavior that D28 explicitly dropped.
- **Collapse everything into one large implementation phase.**
  Rejected because the path-ownership split is a real foundation seam; mixing
  helper/API redesign with end-to-end caller migration would hide whether
  failures came from the low-level model or the high-level consumers.

### Consequence

`plan/overview.md`, `plan/status.md`, and `plan/leaf-ownership.md` now treat the
runtime-home split as the only remaining execution scope. The deleted
workspace-topology phase files are no longer planning authority.

## D25 — Harness Workspace Projection Mechanisms (Clarification)

**Directive (2026-04-17):** Clarifies harness-specific workspace projection mechanisms based on empirical findings:

### Codex

- **Workspace shaping:** `--cd <dir>` (primary root) + `--add-dir <dir>` (extra writable roots). These are workspace projection, not home isolation.
- **Home isolation:** `CODEX_HOME` controls where Codex stores its native state (auth, sessions, config). It is NOT for extra workspace roots.
- **Auth bootstrap:** Fresh isolated `CODEX_HOME` environments empirically require auth bootstrap. Design treats Codex home as a monolithic native-state boundary unless upstream exposes a split auth/state mechanism.
- **Design consequence:** Per D-POST-REVIEW, Codex workspace projection via `--add-dir` is deferred to `harness-permission-abstraction` because `--add-dir` only grants write access, not read access. Full workspace projection requires `CODEX_HOME` config generation with permission profiles.

### OpenCode

- **Workspace shaping:** Launch root (the directory passed to the opencode invocation) plus config/permissions like `permission.external_directory` (via env-projected config such as `OPENCODE_CONFIG_CONTENT`).
- **No `--add-dir` equivalent:** OpenCode has no direct `--add-dir` flag. The extra-path mechanism is permission-based, not topology-based.
- **Design consequence:** OpenCode day-1 projection returns `active:permission_allowlist` with `config_overlay` carrying the `permission.external_directory` patch.

### Claude

- **Workspace shaping:** `--add-dir <dir>` flags. Claude has native `--add-dir` support.
- **Design consequence:** Claude day-1 projection returns `active:add_dir` with `extra_args` carrying the repeated `--add-dir` flags.

## D29 — Simplified UUID Implementation (2026-04-18)

**Decision:** Implemented the simplified UUID-based project identity model.

**Key implementation choices:**

1. **user_paths.py module** — New module owns user state root resolution and project UUID management. Clean separation from repo-level path helpers.

2. **Platform detection** — Reused `IS_WINDOWS` from `meridian.lib.platform` for consistent cross-platform behavior.

3. **Lazy UUID generation** — UUID is generated on first access via `get_or_create_project_uuid()`, not preemptively. Uses `atomic_write_text()` for crash-safe creation.

4. **resolve_runtime_state_root()** — New function in paths.py handles the UUID → user-state-root resolution with MERIDIAN_STATE_ROOT override support.

5. **Bootstrap separation** — Fixed `ensure_runtime_state_bootstrap_sync()` to create:
   - Repo-level: `.meridian/`, `fs/`, `work/`, `work-archive/`
   - Runtime-level: `~/.meridian/projects/<UUID>/`, `spawns/`
   
   This prevents runtime directories from polluting the repo.

6. **StatePaths cleanup** — Removed `artifacts_dir`, `cache_dir`, `spawns_dir` from repo-level `StatePaths` since they belong to runtime state.

**Evidence:** Smoke tests verify:
- Repo `.meridian/` contains only: `id`, `fs/`, `work/`, `work-archive/`, `.gitignore`
- User state at `~/.meridian/projects/<UUID>/` contains runtime state
- Project move preserves UUID and finds existing state
- MERIDIAN_HOME override works
