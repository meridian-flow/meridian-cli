...
### FV-1 Codex `--add-dir` only grants write access (revised 2026-04-17)

**Verdict**: deferred. Codex workspace projection requires `harness-permission-abstraction` work item.

**Evidence**: `codex exec --help` on codex-cli 0.120.0 documents `--add-dir <DIR>` as "Additional directories that should be writable alongside the primary workspace" — note "writable", not "readable".

**Empirical finding (2026-04-17):**
- `--add-dir` only grants write access, NOT read access.
- Read-only directory access requires `CODEX_HOME` config generation with permission profiles.
- Workspace shaping is via `--cd` (primary) + `--add-dir` (extra writable roots).
- `CODEX_HOME` is for native home/state isolation only, not extra workspace roots.

**Residual risk**: Fresh isolated `CODEX_HOME` environments empirically need auth bootstrap. Design treats Codex home as a monolithic native-state boundary unless upstream docs expose a split auth/state mechanism.

**Design consequence**: Per D-POST-REVIEW, Codex workspace projection deferred. V1 returns `unsupported:requires_config_generation`.
...
### FV-13 Project UUID for user-level runtime state

**Verdict**: architecturally sound; implementation-ready per D28.

**Design direction (D28 — UUID simplification):**
- Project identity is a UUID stored in `.meridian/id`
- Generated on first spawn or `meridian init` (UUID v4)
- Moves with the project folder — no derivation algorithm
- User state root: `~/.meridian/` (Unix), `%LOCALAPPDATA%\meridian\` (Windows)
- Repo `.meridian/` holds UUID and committed artifacts only
- **All** runtime state under `~/.meridian/projects/<UUID>/`
- Spawn-id is the universal runtime-home key — primary sessions also receive spawn-ids
- `chat_id` remains resumable-session metadata only, not a storage key
- App `session_id` is a URL-addressable alias, not a storage key

**What moves:**
- `spawns.jsonl` → `~/.meridian/projects/<UUID>/spawns.jsonl`
- `sessions.jsonl` → `~/.meridian/projects/<UUID>/sessions.jsonl`
- `spawns/<spawn-id>/` → `~/.meridian/projects/<UUID>/spawns/<spawn-id>/`
- `cache/` → `~/.meridian/projects/<UUID>/cache/`

**Why UUID instead of derivation**: Previous designs proposed deriving a `project-key` from git remote hashing or path canonicalization. This was complex and fragile. UUID is simpler: generated once, stored in `.meridian/id`, moves with the project folder. No normalization or derivation logic.

**Tradeoff**: Cloning a repo creates a new UUID (the `id` file isn't typically committed). This is acceptable because ephemeral state doesn't need to be shared across clones.

**Design consequence**: `runtime-home.md` (A00) defines UUID model and platform defaults. `paths-layer.md` (A01) includes `UserStatePaths` section. Implementation-ready.

### FV-14 OpenCode extra-path mechanism clarification (2026-04-17)

**Verdict**: confirmed. OpenCode uses permission-based access, not topology-based.

**Empirical finding:**
- OpenCode has no `--add-dir` equivalent flag.
- Extra-path access is via launch root plus config/permissions like `permission.external_directory`.
- Config delivery via `OPENCODE_CONFIG_CONTENT` environment variable.

**Design consequence**: OpenCode projection returns `active:permission_allowlist` with `config_overlay` carrying the permission patch. This is semantically different from Claude/Codex `--add-dir` (which is topology-based) but achieves the same practical result.

### FV-15 User State Root Platform Defaults Are Sound
...
- **Fallback on Windows:** `%USERPROFILE%\AppData\Local\meridian\` if `LOCALAPPDATA` is missing

**Resolution order:**
1. `MERIDIAN_HOME` environment variable if set
2. Platform default per above

**Why these defaults:**
- Unix: `~/.meridian/` is consistent with common CLI tool conventions
- Windows: `%LOCALAPPDATA%` is the standard location for non-roaming user-local application data

**Residual risk**: None. These are well-established platform conventions.

**Design consequence**: `UserStatePaths` module implements platform detection and default resolution.
