...
## D24: `dev-principles` is a shared skill loaded by relevant agents; it is not a binary gate

**Revision note.** This entry revises the prior D24 (r1). The prior correction narrowed `dev-principles` too far to a design-orch convergence checkpoint. This revision preserves the same decision ID and supersedes that framing. **Correction source:** in-session user override in parent session `c1135` during this work item's step B review cycle, not a spawn report — the user ruled that no altitude runs a binary `dev-principles` gate. The revised framing is anchored in `design/architecture/principles/dev-principles-application.md` (A05.1, the R07 anchor) and `design/spec/root-invariants.md` §S00.w1.

**Decision.** `dev-principles` is shared operating guidance loaded by every agent whose work is shaped by structural, refactoring, abstraction, or correctness concerns. It is not a pass/fail checkpoint. Design-orch loads it during convergence as one of several review lenses; convergence is judged by whether findings (including principle violations) are addressed, not by a separate binary gate mechanism. @coder, @reviewer, @architect, and @planner all load and apply the skill continuously in their own work. Impl-orch remains a coordinator and does not need to run its own `dev-principles` check; the agents it spawns already apply the principles. Final implementation review does not need a dedicated `dev-principles` reviewer lane because all reviewers already use the principles as part of their rubric.

**Alternatives rejected.**
- *Treat `dev-principles` as design-orch-only convergence gate.* Rejected because principles do not stop applying at handoff; narrowing them to one checkpoint creates a false boundary between design-time and implementation-time quality.
- *Inline principles separately into each agent body.* Rejected because duplication drifts and weakens shared interpretation over time; a shared skill is the correct representation.
- *Use `dev-principles` as a hard pass/fail gate at any layer.* Rejected because principles are behavioral guidance and judgment lenses, not checklist gates.

**Reasoning.** Principles are universal by definition. Gating them at one orchestration point turns continuous engineering judgment into a false binary. Loading them as a shared skill keeps them operational wherever structure, abstraction, and correctness decisions are made.
...
## D27: Leaf-ID convention, strict-TOC overviews, and reserved root-scope namespace

**Decision.** The two-tree design package (D18) gets three concrete conventions that emerged from dogfooding the step B restructure and are now load-bearing for every future design cycle:

1. **Letter-encoded EARS pattern IDs.** Spec-leaf statement IDs take the form `S<subsystem>.<section>.<letter><number>` where the letter encodes the EARS pattern: `u` = Ubiquitous, `s` = State-driven, `e` = Event-driven, `w` = Where/Optional-feature, `c` = Complex. Architecture-leaf anchor IDs use the same `A<subsystem>.<section>` shape without the EARS letter (architecture is observational, not requirement-shaped). A reader can tell at a glance which parsing rule from D25 applies to a given statement without opening the file.
2. **Strict-TOC overviews.** Every `overview.md` at any altitude in either tree is a strict table of contents: `Purpose` (one paragraph), `TOC` (bulleted list of child leaves with one-line descriptions each), `Reading order` (one paragraph pointing at the entry leaf). No prose narrative, no substantive content, no lifted summaries from leaves. Substance lives in leaves; overviews are navigation. Reviewers reject any overview that carries substance as a strict-TOC violation.
3. **Reserved root-scope namespace.** Root-scope invariants and root-scope structural observations — the claims that apply across every subsystem — live in `design/spec/root-invariants.md` under the reserved `S00.*` namespace and `design/architecture/root-topology.md` under the reserved `A00.*` namespace. Never inline in an overview's prose, never duplicated across subsystems. Subsystems start at `S01.*`/`A01.*` and up.

**Alternatives rejected.**
- *Leaf-only IDs without the EARS letter.* Rejected because a tester or reviewer reading a `plan/leaf-ownership.md` row cannot tell which per-pattern parsing rule from D25 applies without opening every referenced leaf. The letter is a free routing hint at the ID level.
- *Overviews as "short summaries" with a paragraph of context per child.* Rejected because "short summary" is judgment-shaped and drifts toward restating leaf content. The strict TOC rule is mechanical: a `Purpose` paragraph plus a list. Reviewers catch violations without interpreting intent.
- *Inline root-scope claims in each overview.* Rejected because the same invariant then lives in multiple places and drifts. A reserved namespace file makes root-scope claims cite-able from every leaf that depends on them via the `Realizes` / `Realized by` cross-link machinery without duplication.
- *Drop the two-tree distinction and keep a single flat ID namespace.* Already rejected in D18 for the package shape; this decision just extends the ID scheme to match the tree shape.

**Reasoning.** Step B materialized the two-tree package for the first time, and three conventions did work that no amount of design could have decided in the abstract. The letter-encoded IDs showed up the moment the first `plan/leaf-ownership.md` row was written and a tester had to know which parser to run. The strict-TOC rule showed up the moment the first overview drifted toward "let me summarize what's in the leaves." The reserved-namespace rule showed up the moment a root-scope claim (`S00.w1`: `dev-principles` is universal shared guidance) needed a home that was not a subsystem and not an overview. Each convention is small in isolation but compounds: together they make the two-tree package mechanically reviewable without lifting prose interpretation back into the review loop. This decision is derived from the dogfooding during step B of the orchestrator-restructure work item and lives in `design/architecture/design-package/two-tree-shape.md` as the anchor.
