...
- **Refactors and foundational prep land before what they unblock.** Anything named in `design/refactors.md` (interface renames, module reshuffles, shared-helper extraction, decoupling moves) or `design/feasibility.md §"Foundational prep"` (type definitions, abstract base classes, new interface contracts, shared helpers that do not yet exist) is cross-cutting prep that runs before the feature phases that depend on it. The planner reads the `Must land before:` field on each `refactors.md` entry and the "unblocks" notes on each foundational prep entry, and sequences prep work so each item lands in the earliest round that still precedes every phase it unblocks. This is **not** "everything from `refactors.md` must run in Round 1" — cluster-local refactors with narrow scope may bundle with the feature phase they support, and independent refactors on disjoint modules run in parallel within a single prep round. The sequencing rule is "lands before what it unblocks," not "lands earliest regardless of what it unblocks." Getting cross-cutting refactors out of the way early removes the merge-conflict surface that downstream parallelism depends on; over-sequencing cluster-local refactors collapses parallelism into a sequential prep tail. The planner identifies refactors by reading `design/refactors.md` directly; it identifies foundational prep by reading `design/feasibility.md §"Foundational prep"` directly. Neither is invented by the planner (see "The planner does not invent refactors" below).
...
The planner **may not** silently invent foundational prep any more than it can silently invent refactors. Foundational prep is design-orch's artifact (`feasibility.md §"Foundational prep"`), and the single-author rule applies equally: cross-cutting prep in the plan must exist in either `refactors.md` or `feasibility.md §"Foundational prep"`. If pre-planning or planning reveals that missing foundational scaffolding blocks the decomposition, the planner's options are the same two as for a missing refactor:

1. **Probe-request escalation** if the planner is uncertain whether the gap is real or an artifact of incomplete runtime data — use the probe-request channel and let impl-orch resolve it before the next planner spawn.
2. **Structural-blocking escalation** if the gap is real and the plan cannot be written without new scaffolding — terminate with `Parallelism Posture: sequential` and `Cause: structural coupling preserved by design`, name the specific missing foundation in the cause narrative, and let impl-orch route the brief back to design-orch so `feasibility.md` gets the missing entry.

The planner does not append to `feasibility.md` directly, does not silently add prep phases that reference non-existent anchors, and does not "cite a spec leaf" as an in-plan substitute for a design-orch round-trip. Every cross-cutting prep phase the plan spawns must trace to an ID in `refactors.md` or a named item in `feasibility.md §"Foundational prep"`. This is the same traceability chain that refactors enforce; foundational prep is not a side door around it.
...
Impl-orch is allowed to re-spawn the planner when the plan is missing required sections, claims spec leaves that do not exist, leaves `design/refactors.md` entries unmapped, or contradicts the pre-planning notes. To prevent pathological loops where the planner cannot converge, the planning cycle is bounded by **two independent counters** with distinct semantics:

- **`K_fail` — failed-plan counter, capped at 3.** Counts planner spawns that produced a plan but the plan was incomplete, incoherent, or contradictory — missing required sections, spec leaves unclaimed or double-claimed, `refactors.md` entries unmapped, contradiction with pre-planning notes, or hand-wavy parallelism justifications that do not cite real constraints. A failed plan spawn exhausts one slot of `K_fail`. The rationale (D12) is that a planner which cannot converge with the inputs it has will not converge on a fourth try with the same inputs.
- **`K_probe` — probe-request counter, capped at 2.** Counts planner spawns that terminated with a probe-request report rather than a plan, because the planner explicitly identified missing runtime data it could not decide around. A probe-request does **not** advance `K_fail` — the planner is not failing to converge, it is asking for inputs it did not have. The separate cap exists because probe-requests change the inputs (impl-orch runs the new probes and updates `pre-planning-notes.md`), so D12's "same inputs, same failure" logic does not apply. `K_probe=2` still caps the loop so pathological probe-chains cannot run forever.

Exits from the planning loop:

1. **Convergent plan.** A spawn that produces a complete and consistent plan advances neither counter. Impl-orch accepts the plan and moves on to the pre-execution structural gate. This is the normal exit.
2. **Structural-blocking short-circuit.** A spawn that terminates with `Parallelism Posture: sequential` and `Cause: structural coupling preserved by design` (or the equivalent foundational-prep variant) advances **neither** counter. Structural-blocking is a design-side signal, not a planner failure — re-spawning the planner against the same design cannot resolve it. Impl-orch routes the structural-blocking escalation to dev-orch immediately, bypassing the `K_fail`/`K_probe` bookkeeping entirely. This short-circuit must be explicit: if the plan ships with `sequential` + `structural coupling preserved by design`, impl-orch does not re-spawn the planner regardless of remaining cap slots. If both signals apply — a plan is incomplete *and* flags structural-blocking — structural-blocking wins and the re-spawn is skipped.
3. **`K_fail` exhausted.** After three failed plans in a row, impl-orch escalates to dev-orch with a `planning-blocked` signal rather than re-spawning a fourth time. See [impl-orchestrator.md](impl-orchestrator.md) §"Planning cycle cap" for the impl-orch side and `decisions.md` D12 for the rationale.
...
These caps are distinct from the redesign cycle cap (D7). They count separately. Exhausting either planner counter escalates to dev-orch with a different signal than the execution-time falsification escape hatch.
...
- **requirements.md** — the user-facing intent capture (when it exists). The planner reads this so its sequencing respects any user-stated scope, priority, or constraint that the spec leaves and architecture tree do not themselves encode. A work item with no `requirements.md` (trivial or small-path work) simply skips this input; when present it is load-bearing and must not be omitted from the `-f` set.
...
- **design/feasibility.md** — design-time feasibility answers and known unknowns, including the `Foundational prep` section which the planner treats as a second cross-cutting prep input alongside `refactors.md`. The planner reads this so it knows what runtime context design-orch already resolved and does not re-request probes that feasibility.md already answered.
...
- **plan/leaf-ownership.md** — claims are at **EARS-statement granularity**, not leaf-file granularity. Every EARS statement ID in `design/spec/` (e.g. `S03.1.e1`, `S03.1.e2`, `S03.1.e3`) must be claimed by exactly one phase. A leaf file that contains three EARS statements may have its statements split across multiple phases — for example, `S03.1.e1` and `S03.1.e2` claimed by Phase 3 (feature fanout) while `S03.1.e3` (an integration-round invariant) is claimed by Phase 5. The ownership file lists one row per EARS statement, not one row per leaf file. Unclaimed or double-claimed **statements** are a planner bug. When a preservation hint is present (redesign cycle), `leaf-ownership.md` must include entries for preserved phases populated from the hint's "Spec leaves satisfied" column; EARS statements flagged with `revised: <reason>` in the hint get re-verification notes so downstream testers know the response clause changed even though the ID is stable.
