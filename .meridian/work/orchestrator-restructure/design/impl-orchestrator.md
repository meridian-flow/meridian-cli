...
**`dev-principles` convergence gate at the final review.** The same principles gate design-orch applies during design convergence ([design-orchestrator.md](design-orchestrator.md) §"Active structural review during convergence") is re-applied at this stage against the committed implementation. Impl-orch loads `dev-principles` (already in the skills list below) and asks: does the committed change set violate any project-wide structural principle — refactor discipline left unfinished, edge cases present in the spec but not verified, abstractions introduced at fewer than three call sites, structural health signals (module size, import growth, greppability drop) tripped, integration boundaries shipped without protocol probing? A **yes** answer blocks the final review loop from converging until either the violation is resolved in a follow-up phase or an explicit override is recorded in `decisions.md` with reasoning. This is the execution-time counterpart to the design-time gate: the same principles, applied at the stage where the concrete code exists instead of at the stage where only the design exists.
