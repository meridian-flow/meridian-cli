...
### CKSUM-07: Re-sync divergent items
When disk-lock divergence is detected,
the system shall treat divergent items as if the local content changed (triggering re-evaluation in the diff phase rather than skipping them as "unchanged").
...
### CKSUM-09: Target divergence self-healing
...
If target content diverges (missing, different hash, or permission-blocked write that left stale content),
the system shall re-copy from `.mars/` to the target.
