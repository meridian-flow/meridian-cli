...
- Best maintenance recency among candidates (`2025-03-10`).
- Cross-platform implementation is explicit and current.
- Provides direct blocking + non-blocking exclusive semantics (`write` / `try_write`).
- RAII unlock semantics are first-class via guard types.
- Avoids carrying direct `libc` usage in `mars-agents`.
